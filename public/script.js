...
// Use IIFE (Immediately Invoked Function Expression) to create a closure
// This keeps currentForm and editingId scoped, not global
const editPageModule = (function () {
  let currentForm = [];
  let editingId = null;
    ...
    currentForm.forEach((field, i) => {
      ...
      item.innerHTML = `
          ...
          <div class="delete-btn">
            <button class="btn btn-danger btn-small" onclick="event.stopPropagation(); editPageModule.deleteField(${i})">Delete</button>
          </div>
        `;
  ...
  // Return public API - expose functions for onclick handlers
  return {
    loadEditPage,
    addField,
    saveFormAndReturn,
...
*/
// Module for client form page - keeps form data scoped
const clientFormModule = (function () {
  let formData = { formId: null, fields: [] };
  ...
  async function loadClientForm() {
    ...
    try {
      ...
      // Store form data in module scope
  ...
  async function submitClientForm() {
    ...
    // append files
    for (let i = 0; i < formData.fields.length; i++) {
      if (formData.fields[i].type === 'file') {
        const el = document.getElementById(`field-${i}`);
        if (el && el.files) {
    ...
    try {
      const submitRes = await fetch(`/api/forms/${formData.formId}/submissions`, {
        method: 'POST',
        body: fd
      });
      if (!submitRes.ok) {
...
async function renderDashboard() {
  ...
  try {
    ...
    subs.forEach((sub, idx) => {
      ...
      const safeId = escapeHtml(sub._id);
      html += `
          <div class="notes-section">
            <strong>Internal Notes:</strong>
            <div class="notes-display" id="note-display-${safeId}">${safeNotes || '<em>No notes yet.</em>'}</div>
            <textarea id="note-input-${safeId}" style="width:100%; margin-top:10px;" placeholder="Add notes...">${safeNotes}</textarea><br>
            <button class="btn btn-primary btn-small" onclick="saveNote('${safeId}')">Save Note</button>
            <button class="btn btn-danger btn-small" onclick="deleteSubmission('${safeId}')">Delete Submission</button>
          </div>
        `;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (path.includes('index.html') || path.endsWith('/') || path.endsWith('/public')) {
    ...
    editPageModule.loadEditPage();
  } else if (path.includes('form.html')) {
    clientFormModule.loadClientForm();
